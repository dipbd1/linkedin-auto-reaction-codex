...
Use this skill to assist a LinkedIn engagement session in the native Windows or macOS Codex app. Operate through the Codex Chrome plugin/extension in the user's signed-in Chrome browser, and use `@computer` only when native UI interaction is needed. Prefer the user's existing logged-in Chrome session when available.

The Codex Chrome plugin/extension must be installed, enabled, and connected before this skill can review LinkedIn pages properly. If it is unavailable, ask the user to set it up from `https://developers.openai.com/codex/app/chrome-extension` and continue only with pasted-text drafting until Chrome is connected.
...
1. Use the visible LinkedIn UI in the user's Chrome browser only, controlled through the Codex Chrome plugin/extension or `@computer`.
2. Prefer the currently selected Chrome tab/session when it is already logged in.
...
6. Never use stealth, proxy, fingerprint evasion, hidden APIs, Voyager endpoints, copied cookies, non-Codex extensions, or direct HTTP calls to LinkedIn.
...
1. **Setup**
   - Use the Codex Chrome plugin/extension to open or attach to LinkedIn in the user's Chrome browser.
   - Use `@computer` only when Chrome cannot access or operate the needed visible browser UI.
   - Reuse the selected/current Chrome tab if it is already on LinkedIn or already logged in.
...
## Codex Chrome browser protocol

For detailed browser instructions, read `references/native-machine-browser.md` only when operating the browser.

Core rules:

1. Use the Codex Chrome plugin/extension first for navigation, page inspection, screenshots, form filling, scrolling, and visible UI clicks in the user's Chrome browser.
2. Use `@computer` as a fallback for OS-level browser selection, permission prompts, or visible UI controls unavailable through Chrome.
3. Reuse the selected Chrome tab and logged-in session when possible. Do not launch a hidden, remote, or separate automation-only browser profile.
4. Inspect visible page state through Chrome snapshots/screenshots, or with `@computer` screenshots when operating native UI.
...
- `references/native-machine-browser.md`: native Windows/macOS Chrome workflow using the Codex Chrome plugin/extension and `@computer`.
